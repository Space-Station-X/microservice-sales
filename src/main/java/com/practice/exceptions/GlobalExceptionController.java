...
public class GlobalExceptionController {
    ...
    public ResponseEntity<ErrorResponse> handleClientNotFoundException(SalesNotFoundException ex) {
        ErrorResponse errorResponse =
                new ErrorResponse(
                        "SALES_ERROR",
                        "Ha ocurrido un error con las ventas",
                        Collections.singletonList(ex.getMessage())
                );
        log.warn("Sales not found : {}", ex.getMessage());
        return ResponseEntity.status(HttpStatus.NOT_FOUND).body(errorResponse);
    }
